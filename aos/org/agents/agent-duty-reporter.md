...
channels:        [feishu, aos-commit, cron]
...
| 仓库 Wiki | 文件 | 读 `aos/runtime/tickets/open` 统计当前 open 工单数 | 每次运行 |
| 昨日日报 | 文件 | 读 `aos/reports/project/daily/` 昨日文件作对比 | 每次运行 |
...
| 产出类型 | 格式 | 落地位置 | 命名 |
|---------|------|----------|------|
| 每日日报 | Markdown | `aos/reports/project/daily/YYYY-MM-DD.md` | 每日独立文件 + `_latest.md` 指针 |
| 异常告警 | 文本 | feishu | 即时推送 |
| Git commit | — | aos 仓库 | `report(daily): YYYY-MM-DD` |
...
- 业务数据库 ❌
### 4.2 写权限
- aos/reports/project/daily/* ✅ 每日新增（独占写，含 _latest.md）
- docs/**（其他） ❌
- aos/runtime/tickets/** ❌（本员工不创建工单）
- 业务系统任何端点 ❌

### 4.3 执行权限
- ✅ 调用健康 API（只读）
- ✅ 向 feishu 推送消息
- ✅ 向 aos 仓库提交 commit（仅限 reports/project/daily/）
- ❌ 任何写数据库、调部署、改配置的动作
...
### 5.3 汇报策略
- **每日 08:00**：日报推 feishu + 写 `reports/project/daily/`
...
| 0.1.1 | 2026-04-29 | 日报路径改 `aos/reports/project/daily/`；通道 Telegram 残留修正为 feishu；Git commit 格式对齐 SKILL.md | Boss |
